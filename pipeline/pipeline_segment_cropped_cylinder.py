...
class PredictionDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i):
        ...
        names = self.image_paths[i].split('/', 3)[-1].split('.')[0] # split start from the wave
...
def main():
    ...
    # crop images 
    def crop_img(image_path,crop_folder):
        ...
        for wave in wave_list:
            ...
            for scanner in scanner_list:
                ...
                startY = 180 if scanner.endswith('SlowScanner') else 219 # based on the 1030 Y for slowscanner, 1069Y for FastScannner
                startX = 526 if scanner.endswith('SlowScanner') else 386
                height = 850
                width = 970

                #if scanner.endswith('FastScanner'):
                #    startX = 400
                #    startY = 220
                #if scanner.endswith('SlowScanner'):
                #    startX = 550
                #    startY = 190
                ...
                for plant in plant_list:
                    ...
                    for img in sorted_img_list:
                        ...
                        # image_crop = image[startY:startY+850, startX:startX+970, :]
                        # cv2.imwrite(os.path.join(crop_folder,img),image_crop)

                        # get the boundary layer
                        img_gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
                        img_crop = img_gray[startY:startY+height, startX+50:startX+width-50] # remove the left and right 50 pixels
                        ...
                        ind = np.argmin(moving_averages[150:-50]) + startY + int(window_size/2) + 150 # filter out the first 150 rows and last 50 rows
    ...
    crop_img(image_path,crop_folder)                
    ...
    for idx in range(1): # len(test_dataset)
        ...
        cv2.imwrite(os.path.join(sample_preds_folder, subfolder,f"{img_name}.png"), pred_mask)
